    ...
    if len(uploaded_files) > 2:
                        ...
                        if suffix == ".pdf":
                                ...
                                for page in pdf.pages:
                                    ...
                                    for table in tables:
                                        if table and len(table) > 0:
                                            text_list.append("\n")
                                            # Clean table rows and cells safely
                                            clean_headers = [str(c or '').strip() for c in table[0]]
                                            text_list.append("| " + " | ".join(clean_headers) + " |")
                                            text_list.append("| " + " | ".join(["---"] * len(clean_headers)) + " |")
                                            for row in table[1:]:
                                                clean_row = [str(c or '').strip() for c in row]
                                                text_list.append("| " + " | ".join(clean_row) + " |")
                                            text_list.append("\n")
                        else:
                            ...
                            for table in doc.tables:
                                if table.rows and len(table.rows) > 0:
                                    text_list.append("\n")
                                    # Create table matrix safely
                                    grid = [[cell.text.strip() for cell in row.cells] for row in table.rows]
                                    
                                    # FIXED: Correctly reference the index 0 row string list for the header
                                    header_cells = grid[0]
                                    text_list.append("| " + " | ".join(header_cells) + " |")
                                    text_list.append("| " + " | ".join(["---"] * len(header_cells)) + " |")
                                    
                                    # FIXED: Correctly loop through each row string list for data rows
                                    for r in grid[1:]:
        ...
        if len(uploaded_files) == 2 and 0 in converted_markdowns and 1 in converted_markdowns:
            ...
            with diff_cols[0]:
                ...
                st.text_area(
                    label="Left Stream",
                    value=converted_markdowns[0],  # FIXED: Pulls only Document 1
                    height=650,
                    key="side_view_doc1",
            ...
            with diff_cols[1]:
                ...
                st.text_area(
                    label="Right Stream",
                    value=converted_markdowns[1],  # FIXED: Pulls only Document 2
                    height=650,
                    key="side_view_doc2",
                    label_visibility="collapsed"
                )
